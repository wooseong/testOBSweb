...
struct ffmpeg_muxer {
	obs_output_t *output;
	os_process_pipe_t *videoPipe;
	os_process_pipe_t *audioPipe;
	int64_t stop_ts;
	uint64_t total_bytes;
	struct dstr videoPath;
	struct dstr audioPath;
	bool sent_video_headers;
	bool sent_audio_headers;
...
static void ffmpeg_mux_destroy(void *data)
{
	...
	os_process_pipe_destroy(stream->videoPipe);
	dstr_free(&stream->videoPath);
	dstr_free(&stream->audioPath);
	bfree(stream);
}
...
#ifdef _WIN32
#define FFMPEG_MUX "todo-plugin-mux.exe"
#else
#define FFMPEG_MUX "todo-plugin-mux"
#endif
...
static void build_video_command_line(struct ffmpeg_muxer *stream,
				     struct dstr *cmd, const char *path)
{
	...
	dstr_copy(&stream->videoPath, path);
	dstr_replace(&stream->videoPath, "\"", "\"\"");
	dstr_cat_dstr(cmd, &stream->videoPath);

	dstr_catf(cmd, "\" %d %d ", vencoder ? 1 : 0, 0);
...
static void build_audio_command_line(struct ffmpeg_muxer *stream,
				     struct dstr *cmd, const char *path)
{
	...
	dstr_copy(&stream->audioPath, path);
	dstr_replace(&stream->audioPath, "\"", "\"\"");
	dstr_cat_dstr(cmd, &stream->audioPath);

	dstr_catf(cmd, "\" %d %d ", 0, num_tracks);
...
static inline void start_pipe(struct ffmpeg_muxer *stream, const char *path)
{
	...
	stream->videoPipe = os_process_pipe_create(cmd.array, "w");
	dstr_free(&cmd);
}

static inline void start_pipe_separate(struct ffmpeg_muxer *stream,
				       const char *videoPath,
				       const char *audioPath)
{
	struct dstr videoCmd, audioCmd;
	build_video_command_line(stream, &videoCmd, videoPath);
	blog(LOG_INFO, videoCmd.array);
	stream->videoPipe = os_process_pipe_create(videoCmd.array, "w");
	dstr_free(&videoCmd);

	build_audio_command_line(stream, &audioCmd, audioPath);
	blog(LOG_INFO, audioCmd.array);
	stream->audioPipe = os_process_pipe_create(audioCmd.array, "w");
	dstr_free(&audioCmd);
}

static bool
ffmpeg_mux_start(void *data) /* output->info.start video, audio 섞는 것 */
{ /*ffmpeg_muxer context.data(void *data) 는 info.create 시 리턴밸류*/
	struct ffmpeg_muxer *stream = data;
	obs_data_t *settings;
	const char *videoPath;
	const char *audioPath;
	...
	videoPath = "c:\\videos\\my.flv";
	audioPath = "c:\\audios\\my.flv";
	...
	FILE *test_file = os_fopen(videoPath, "wb"),
	     *test_audio_file = os_fopen(audioPath, "wb");
	if (!test_file || !test_audio_file) {
...
#endif
		dstr_replace(&error_message, "%1", videoPath);
	...
	start_pipe_separate(stream, videoPath, audioPath);
	obs_data_release(settings);
	...
	info("Writing video '%s'...", stream->videoPath.array);
	info("Writing audio '%s'...", stream->audioPath.array);
	return true;
}
...
static int deactivate(struct ffmpeg_muxer *stream, int code)
{
	...
	if (active(stream)) {
		ret = os_process_pipe_destroy(stream->videoPipe);
		stream->videoPipe = NULL;

		os_atomic_set_bool(&stream->active, false);
		os_atomic_set_bool(&stream->sent_video_headers, false);
		os_atomic_set_bool(&stream->sent_audio_headers, false);

		info("Output of video '%s' stopped", stream->videoPath.array);
		info("Output of audio '%s' stopped", stream->audioPath.array);
...
static void signal_failure(struct ffmpeg_muxer *stream)
{
	...
	len = os_process_pipe_read_err(stream->videoPipe, (uint8_t *)error,
				       sizeof(error) - 1);
			 ...
			 struct encoder_packet *packet)
{
	...
	if (is_video) {
		ret = os_process_pipe_write(stream->videoPipe,
					    (const uint8_t *)&info,
					    sizeof(info));
		if (ret != sizeof(info)) {
		...
		ret = os_process_pipe_write(stream->videoPipe, packet->data,
					    packet->size);
		if (ret != packet->size) {
		...
		ret = os_process_pipe_write(stream->audioPipe,
					    (const uint8_t *)&info,
					    sizeof(info));
		if (ret != sizeof(info)) {
			warn("os_process_pipe_write for info structure failed");
		...
		ret = os_process_pipe_write(stream->audioPipe, packet->data,
					    packet->size);
		if (ret != packet->size) {
			warn("os_process_pipe_write for packet data failed");
			signal_failure(stream);
...
static bool send_audio_headers_all(struct ffmpeg_muxer *stream) {
...
static void ffmpeg_mux_data(void *data, struct encoder_packet *packet)
{
	...
	if (!stream->sent_video_headers) {
		if (!send_video_headers(stream))
			return;

		stream->sent_video_headers = true;
	}

	if (!stream->sent_audio_headers) {
		if (!send_audio_headers_all(stream))
			return;

		stream->sent_audio_headers = true;
	}
...
struct obs_output_info todo_output = {
	.id = "todo_output",
	.flags = OBS_OUTPUT_AV | OBS_OUTPUT_ENCODED | OBS_OUTPUT_MULTI_TRACK,
	.get_name = ffmpeg_mux_getname,
	.create = ffmpeg_mux_create,
	.destroy = ffmpeg_mux_destroy,
	.start = ffmpeg_mux_start,         /* 시작하는 함수 */
	.stop = ffmpeg_mux_stop,           /* 멈추는 함수*/
	.encoded_packet = ffmpeg_mux_data, /* 패킷이 왔을 때 */
...
static void get_last_replay(void *data, calldata_t *cd)
{
	...
	if (!os_atomic_load_bool(&stream->muxing))
		calldata_set_string(cd, "path", stream->audioPath.array);
}
...
static void *replay_buffer_mux_thread(void *data)
{
	...
	start_pipe(stream, stream->videoPath.array);
	...
	info("Wrote replay buffer to '%s'", stream->videoPath.array);
...
static void replay_buffer_save(struct ffmpeg_muxer *stream)
{
	...
	dstr_copy(&stream->videoPath, dir);
	dstr_replace(&stream->videoPath, "\\", "/");
	if (dstr_end(&stream->videoPath) != '/')
		dstr_cat_ch(&stream->videoPath, '/');
	dstr_cat(&stream->videoPath, filename);
