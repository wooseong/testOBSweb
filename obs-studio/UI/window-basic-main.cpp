...
void OBSBasic::StartRecording() /* 여기서 레코딩 시작 */
{
	if (outputHandler->RecordingActive()) /* 기본적으로 SimpleOutput->RecordingActive(), 설정에서 고급으로 변경시 AdvancedOutput->RecordingActive() */
		return;
	if (disableOutputsRef)
		return;

	if (api) /* obs_frontend_callbacks */
