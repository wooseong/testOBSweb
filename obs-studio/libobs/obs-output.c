...
bool obs_output_actual_start(obs_output_t *output) /* fileoutput */
{
	bool success = false;
	...
	if (output->context.data)
		blog(LOG_INFO, "%s", output->info.get_name);
		success = output->info.start(output->context.data); /*
								    여기?, fileOutput
								    ffmpeg_muxer
								    context.data 는 info.create 시 리턴밸류
								    */
...
bool obs_output_start(obs_output_t *output)
{
	...
	if (encoded && output->delay_sec) {
		...
		if (obs_output_actual_start(output)) { /* 여기서 ㄹㅇ 시작 */
